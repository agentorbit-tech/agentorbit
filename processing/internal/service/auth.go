...
type AuthService struct {
	...
	// bcryptSem caps concurrent bcrypt operations at runtime.NumCPU() so a
	// burst of unauthenticated requests cannot pin every CPU core in a single
	// scrypt-tier hashing round and starve the rest of the API. Acquire with
	// bcryptAcquire() before any bcrypt call and pair every successful
	// acquire with bcryptRelease() (typically via defer).
...
func NewAuthService(ctx context.Context, queries *db.Queries, pool *pgxpool.Pool, mailer email.Mailer, jwtSecret, hmacSecret, deploymentMode string, jwtTTL time.Duration, skipEmailVerification bool) *AuthService {
	...
	return &AuthService{
		...
		bcryptSem:             make(chan struct{}, bcryptCap),
	}
}

// bcryptAcquire blocks until a bcrypt slot is available, ctx is cancelled,
// or 2 seconds pass. The 2s ceiling lets pre-auth requests fail fast with a
// 503 "auth_busy" instead of pile up indefinitely during a burst.
func (s *AuthService) bcryptAcquire(ctx context.Context) error {
	select {
	...
	case <-time.After(2 * time.Second):
		return &ServiceError{
			...
			Code:    "auth_busy",
			Message: "Authentication service is busy. Please try again.",
		}
	}
}

// bcryptRelease frees a bcrypt slot. Always pair with a prior successful
// bcryptAcquire — never call on a context-cancel / timeout path.
func (s *AuthService) bcryptRelease() {
	<-s.bcryptSem
}
...
func (s *AuthService) Register(ctx context.Context, emailAddr, name, password, locale, orgName string) (*RegisterResult, error) {
	...
	// Hash password (semaphore-bounded to avoid CPU exhaustion).
...
func (s *AuthService) Login(ctx context.Context, emailAddr, password string) (*LoginResult, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			...
			// Semaphore-bounded so the dummy path obeys the same back-pressure
			// as the real path.
			if acqErr := s.bcryptAcquire(ctx); acqErr != nil {
				return nil, acqErr
			}
	...
	// Check password (semaphore-bounded).
	if acqErr := s.bcryptAcquire(ctx); acqErr != nil {
		return nil, acqErr
	}
	pwErr := crypto.CheckPassword(password, user.PasswordHash)
	s.bcryptRelease()
	if pwErr != nil {
		s.loginLimiter.record(emailAddr)
		return nil, &ServiceError{Code: "invalid_credentials", Message: "Invalid email or password", Status: http.StatusUnauthorized}
...
func (s *AuthService) ResetPassword(ctx context.Context, token, newPassword string) error {
	...
	// Hash new password (before transaction, semaphore-bounded).
	if acqErr := s.bcryptAcquire(ctx); acqErr != nil {
		return acqErr
	}
...
func (s *AuthService) ChangePassword(ctx context.Context, userID uuid.UUID, currentPassword, newPassword string) error {
	...
	// Verify current password (semaphore-bounded).
	if acqErr := s.bcryptAcquire(ctx); acqErr != nil {
		return acqErr
	}
	pwErr := crypto.CheckPassword(currentPassword, user.PasswordHash)
	s.bcryptRelease()
	if pwErr != nil {
		return &ServiceError{Status: http.StatusUnauthorized, Code: "wrong_password", Message: "Current password is incorrect"}
	}

	// Hash new password (semaphore-bounded).
	if acqErr := s.bcryptAcquire(ctx); acqErr != nil {
		return acqErr
	}
