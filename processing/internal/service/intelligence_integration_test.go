...
func TestIntelligenceService_ExtractSystemPrompts(t *testing.T) {
	...
	// Get the session
	dashSvc := service.NewDashboardService(queries, pool, pool, 100000)
...
func TestIntelligenceService_ExtractSystemPrompts_ShortPrefix(t *testing.T) {
	...
	sessions, err := service.NewDashboardService(queries, pool, pool, 100000).ListSessions(ctx, org.ID, service.ListSessionsParams{})
	if err != nil {
		t.Fatalf("list sessions: %v", err)
	}
	sessionID := sessions.Sessions[0].ID
...
func TestIntelligenceService_ExtractSystemPrompts_SingleSpan(t *testing.T) {
	...
	sessions, _ := service.NewDashboardService(queries, pool, pool, 100000).ListSessions(ctx, org.ID, service.ListSessionsParams{})
	sessionID := sessions.Sessions[0].ID
