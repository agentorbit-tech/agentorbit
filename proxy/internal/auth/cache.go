...
//
// ProviderKey is stored as a Secret so any default fmt/JSON path renders
// "[redacted]". Reveal() is called only at provider-header-set sites in
// the proxy handler.
...
func (c *AuthCache) fetchFromProcessing(ctx context.Context, digest string) (*AuthVerifyResult, error) {
	...
	if result.Valid && (result.ProviderType == "" || result.BaseURL == "" || result.ProviderKey.Reveal() == "") {
