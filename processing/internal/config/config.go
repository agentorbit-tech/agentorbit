...
type ProcessingConfig struct {
	...
	// Error tracking (optional — empty SentryDSN disables)
	SentryDSN         string
...
func LoadProcessing() (*ProcessingConfig, error) {
	...
	cfg.SentryEnvironment = getEnvOrDefault("SENTRY_ENVIRONMENT", "self-host")
	cfg.SentryRelease = os.Getenv("SENTRY_RELEASE")
	cfg.SentrySampleRate = getFloatOrDefault("SENTRY_SAMPLE_RATE", 1.0)
	...
	if cfg.SMTPHost != "" {
		if err := rejectPlaceholder("SMTP_PASS", cfg.SMTPPass); err != nil {
			return nil, err
		}
	}
	if cfg.LLMAPIKey != "" {
		if err := rejectPlaceholder("PROCESSING_LLM_API_KEY", cfg.LLMAPIKey); err != nil {
			return nil, err
		}
	}
