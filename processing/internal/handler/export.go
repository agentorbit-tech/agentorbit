...
func (h *DashboardHandler) ExportSessions(w http.ResponseWriter, r *http.Request) {
	...
	// Detach the export query from the per-handler 10s deadline applied to /api.
	// Large CSV exports legitimately exceed 10s; correctness of the export
	// outweighs client-disconnect cancellation here. Bound at the DB layer by
	// a longer statement_timeout on the dedicated export pool (SP-1 Task 6).
	exportCtx := context.WithoutCancel(r.Context())

	switch level {
	case "sessions":
		rows, truncated, err := h.dashboardService.ExportSessions(exportCtx, orgID, params)
		if err != nil {
	...
	case "spans":
		rows, truncated, err := h.dashboardService.ExportSpans(exportCtx, orgID, params)
		if err != nil {
			cw.Flush()
			return
		}
