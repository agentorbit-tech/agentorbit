...
func TestDrainOnShutdown(t *testing.T) {
	...
	// Start workers, then call Drain explicitly — Drain should let workers
	// process all buffered items before returning.
...
func TestWorkerStopsOnCancel(t *testing.T) {
	...
	// Drain triggers worker stop via channel close (or workerCancel after deadline).
	d.Drain(2 * time.Second)

	// workersDone should be closed once workers exit.
	select {
	case <-d.workersDone:
		// Good — workers exited.
	case <-time.After(time.Second):
		t.Fatal("workers did not stop after Drain")
	}

	// Dispatch should not block even though dispatcher is drained
	done := make(chan struct{})
	go func() {
		d.Dispatch(SpanPayload{APIKeyID: "x"})
		close(done)
	...
	case <-done:
		// Good
	case <-time.After(100 * time.Millisecond):
		t.Fatal("Dispatch blocked after Drain")
	}
}

func TestSpanDispatcher_DrainAllBufferedSpans(t *testing.T) {
	...
	d := NewSpanDispatcher(srv.URL, "tok", 5000, srv.Client(), 5*time.Second, 5*time.Second, 0)
	...
	d.Drain(15 * time.Second)
	if got.Load() != 1000 {
		t.Fatalf("processed %d spans, want 1000", got.Load())
	}
}

func TestSpanDispatcher_DispatchAfterDrainDrops(t *testing.T) {
	d := NewSpanDispatcher("http://example.invalid", "tok", 10, nil, time.Second, time.Second, 0)
	d.Start(context.Background())
	d.Drain(100 * time.Millisecond)
	before := d.Dropped()
	d.Dispatch(SpanPayload{APIKeyID: "x"})
	if d.Dropped() != before+1 {
		t.Fatalf("post-drain Dispatch did not drop: before=%d after=%d", before, d.Dropped())
	}
}

func TestSpanDispatcher_DrainIdempotent(t *testing.T) {
	d := NewSpanDispatcher("http://example.invalid", "tok", 10, nil, time.Second, time.Second, 0)
	d.Start(context.Background())
	d.Drain(50 * time.Millisecond)
	d.Drain(50 * time.Millisecond) // must not panic
}
