...
// encodeAuthResult writes a wire-faithful JSON envelope so test servers can
// emit a real provider_key. Default JSON marshaling of AuthVerifyResult would
// redact the Secret value, which is correct in production but defeats tests
// that want to round-trip a known plaintext through the proxy auth cache.
func encodeAuthResult(w http.ResponseWriter, r AuthVerifyResult) {
	wire := struct {
...
func TestHMACDigest(t *testing.T) {
	// Known test vector: HMAC-SHA256 of "ao-abc123" with secret "testsecret"
	digest := hmacDigest("ao-abc123", "testsecret")
	expected := testHMACDigest("ao-abc123", "testsecret")
	if digest != expected {
...
func TestCacheHit(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount.Add(1)
		encodeAuthResult(w, AuthVerifyResult{Valid: true, APIKeyID: "key-1", ProviderType: "openai", BaseURL: "https://api.openai.com", ProviderKey: NewSecret("sk-test")})
	}))
	defer srv.Close()
	...
	digest := hmacDigest("ao-mykey", "secret")
	cache.mu.Lock()
	...
	result, err := cache.Lookup(context.Background(), "ao-mykey")
	if err != nil {
...
func TestCacheMiss(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount.Add(1)
		encodeAuthResult(w, AuthVerifyResult{Valid: true, APIKeyID: "fetched-id", ProviderType: "openai", BaseURL: "https://api.openai.com", ProviderKey: NewSecret("sk-test")})
	}))
	defer srv.Close()

	cache := NewAuthCache(context.Background(), srv.URL, "tok", "secret", 10*time.Second, srv.Client(), 0)

	result, err := cache.Lookup(context.Background(), "ao-newkey")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCacheExpiry(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount.Add(1)
		encodeAuthResult(w, AuthVerifyResult{Valid: true, APIKeyID: "id", ProviderType: "openai", BaseURL: "https://api.openai.com", ProviderKey: NewSecret("sk-test")})
	}))
	defer srv.Close()
	...
	_, err := cache.Lookup(context.Background(), "ao-key")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if callCount.Load() != 1 {
...
func TestFailOpenStaleEntry(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encodeAuthResult(w, AuthVerifyResult{Valid: true, APIKeyID: "stale-id", ProviderType: "openai", BaseURL: "https://api.openai.com", ProviderKey: NewSecret("sk-test")})
	}))
	...
	result, err := cache.Lookup(context.Background(), "ao-key")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFailOpenNoEntry(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encodeAuthResult(w, AuthVerifyResult{Valid: true, ProviderType: "openai", BaseURL: "https://api.openai.com", ProviderKey: NewSecret("sk-test")})
	}))
...
func TestInvalidKeyNotCached(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount.Add(1)
		encodeAuthResult(w, AuthVerifyResult{Valid: false, Reason: "invalid_key"})
	}))
	defer srv.Close()
	...
	// First call
	result, err := cache.Lookup(context.Background(), "ao-badkey")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	_, err = cache.Lookup(context.Background(), "ao-badkey")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if callCount.Load() != 2 {
...
func TestCircuitBreakerOpens(t *testing.T) {
	...
	for i := 0; i < cbFailThreshold; i++ {
		_, _ = cache.Lookup(context.Background(), "ao-key"+string(rune('0'+i)))
	}

	// Circuit should now be open — next call should fail fast without hitting server
	beforeCount := callCount.Load()
	_, err := cache.Lookup(context.Background(), "ao-newkey")
	if err == nil {
...
func TestCircuitBreakerServesStaleWhenOpen(t *testing.T) {
	...
	// Populate cache
	_, err := cache.Lookup(context.Background(), "ao-mykey")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestEvictLoop(t *testing.T) {
	...
	// Populate cache
	_, err := cache.Lookup(context.Background(), "ao-evictme")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestLRUEviction(t *testing.T) {
	...
	// Fill to capacity
	_, _ = cache.Lookup(context.Background(), "ao-key1aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
	time.Sleep(5 * time.Millisecond) // ensure different lastUsedAt
	_, _ = cache.Lookup(context.Background(), "ao-key2aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
	time.Sleep(5 * time.Millisecond)

	// This should trigger LRU eviction of key1 (oldest lastUsedAt)
	_, _ = cache.Lookup(context.Background(), "ao-key3aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
...
func TestFetchFromProcessing_MissingProviderFields(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		encodeAuthResult(w, AuthVerifyResult{Valid: true, APIKeyID: "key-1"})
	}))
	defer srv.Close()

	cache := NewAuthCache(context.Background(), srv.URL, "tok", "secret", 10*time.Second, srv.Client(), 0)

	_, err := cache.Lookup(context.Background(), "ao-abcdef1234567890abcdef1234567890")
	if err == nil {
...
func TestFailOpenStaleInvalidEntry(t *testing.T) {
	...
	digest := hmacDigest("ao-badkey-stale", "secret")
	cache.mu.Lock()
	cache.entries[digest] = cacheEntry{
	...
	result, err = cache.Lookup(context.Background(), "ao-badkey-stale")
	if err == nil {
...
func TestMaxStaleDurationExpired(t *testing.T) {
	...
	// Populate cache
	_, err := cache.Lookup(context.Background(), "ao-stalekey")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	digest := hmacDigest("ao-stalekey", "secret")
	cache.mu.Lock()
	...
	result, err := cache.Lookup(context.Background(), "ao-stalekey")
	if err == nil {
