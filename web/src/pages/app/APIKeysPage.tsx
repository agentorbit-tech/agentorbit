...
export function APIKeysPage() {
  ...
  const [testResult, setTestResult] = useState<{ keyID: string; result?: TestKeyResult; error?: string } | null>(null)
  const [rawKeyTest, setRawKeyTest] = useState<{ loading: boolean; result?: TestKeyResult; error?: string }>({ loading: false })
  const [rawKeyTestModel, setRawKeyTestModel] = useState('')

  const KNOWN_PROVIDERS = ['openai', 'anthropic', 'deepseek', 'mistral', 'groq', 'gemini']

  async function handleTest(keyID: string, providerType: string) {
    const needsModel = !KNOWN_PROVIDERS.includes(providerType)
    if (needsModel && !testModel.trim()) {
    ...
    setTestingKeyID(keyID)
    try {
      const result = await testKey.mutateAsync({ keyID, model: needsModel ? testModel.trim() : undefined })
      setTestResult({ keyID, result })
    } catch (err) {
      const message = err instanceof ApiError ? err.message : 'Test failed'
      setTestResult({ keyID, error: message })
    }
  }
  ...
  async function handleRawKeyTest() {
    ...
    const needsModel = !KNOWN_PROVIDERS.includes(rawKeyResult.provider_type)
    if (needsModel && !rawKeyTestModel.trim()) return
    setRawKeyTest({ loading: true })
    try {
      const result = await testKey.mutateAsync({ keyID: rawKeyResult.id, model: needsModel ? rawKeyTestModel.trim() : undefined })
      setRawKeyTest({ loading: false, result })
    } catch (err) {
      const message = err instanceof ApiError ? err.message : 'Test failed'
      setRawKeyTest({ loading: false, error: message })
    }
  }
                            ...
                            {testingKeyID === key.id && !KNOWN_PROVIDERS.includes(key.provider_type) && !testResult ? (
                              <div className="flex items-center gap-1.5">
                                <input
                                  ...
                                  placeholder={t.keys_test_model_placeholder}
                                  className="w-32 bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500"
                                  onKeyDown={(e) => { if (e.key === 'Enter' && testModel.trim()) handleTest(key.id, key.provider_type) }}
                                  autoFocus
                                />
                                <button
                                  onClick={() => handleTest(key.id, key.provider_type)}
                                  disabled={!testModel.trim() || testKey.isPending}
                                  className="text-xs text-blue-400 hover:text-blue-300 disabled:opacity-50 transition-colors"
                                >
                                  <Play size={12} />
                                </button>
                                <button
                                  onClick={() => { setTestingKeyID(null); setTestModel('') }}
                                  className="text-xs text-zinc-600 hover:text-zinc-400 transition-colors"
                                >✕</button>
                              </div>
                            ) : testKey.isPending && testingKeyID === key.id ? (
                              <span className="inline-flex items-center gap-1.5 text-xs text-blue-400">
                                <Loader2 size={12} className="animate-spin" />{t.keys_test_testing}
                              </span>
                            ) : testResult?.keyID === key.id ? (
                              testResult.result ? (
                                <span className="inline-flex items-center gap-1.5 text-xs text-emerald-400" title={testResult.result.response}>
                                  <Check size={12} />{t.keys_test_success}
                                </span>
                              ) : (
                                <span className="text-xs text-red-400 max-w-xs break-words" title={testResult.error}>
                                  {testResult.error}
                                </span>
                              )
                            ) : (
          ...
          <div className="py-2 space-y-4">
            ...
            {rawKeyResult && !KNOWN_PROVIDERS.includes(rawKeyResult.provider_type) && !rawKeyTest.result ? (
              <input
                ...
                onChange={(e) => setRawKeyTestModel(e.target.value)}
                placeholder={t.keys_test_model_placeholder}
                disabled={rawKeyTest.loading}
                onKeyDown={(e) => { if (e.key === 'Enter' && rawKeyTestModel.trim() && !rawKeyTest.loading) handleRawKeyTest() }}
                className="w-full bg-zinc-950 border border-zinc-800 rounded-md px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600 focus:ring-1 focus:ring-zinc-600 transition-colors disabled:opacity-50"
              />
            ) : null}
            {rawKeyTest.result ? (
              <div className="flex items-center gap-2 text-sm text-emerald-400">
                <Check size={14} />{t.keys_test_success}
              </div>
            ) : rawKeyTest.error ? (
              <p className="text-sm text-red-400 break-words">{rawKeyTest.error}</p>
            ) : null}
          </div>
          <DialogFooter>
            <button
              onClick={handleRawKeyTest}
              disabled={rawKeyTest.loading || !!rawKeyTest.result || (rawKeyResult ? !KNOWN_PROVIDERS.includes(rawKeyResult.provider_type) && !rawKeyTestModel.trim() : false)}
              className="text-sm font-medium text-blue-400 hover:text-blue-300 disabled:opacity-50 transition-colors flex items-center gap-1.5"
            >
