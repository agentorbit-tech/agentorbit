...
// keyTokenRegex matches AgentOrbit / OpenAI / Anthropic-style API keys with
// >= 20 chars of alphanumeric body. The 20-char floor avoids collateral
// matches on short prefixes like "sk-" appearing in error messages.
var keyTokenRegex = regexp.MustCompile(`(sk-|ao-)[a-zA-Z0-9]{20,}`)

// bearerRegex matches the Authorization Bearer scheme followed by any
// non-whitespace token. Used to redact bearer tokens that leak into log
// messages or extra/tag values.
...
// scrubEvent strips sensitive headers, body, and stack-frame locals before
// the event is sent to Sentry. Idempotent and nil-safe.
	...
	if event.Request != nil {
		if event.Request.Headers != nil {
			for name := range event.Request.Headers {
				lower := name
				// canonicalize to lowercase via simple ascii fold
