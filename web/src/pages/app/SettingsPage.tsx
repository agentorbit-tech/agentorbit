...
import { useProRequestStatus } from '@/hooks/use-billing'
...
// Shown next to the plan badge in the General tab. Three states:
//   - billing disabled (self-host) or non-free plan: render nothing
//   - already requested: show "we'll be in touch" pill with date
//   - free + not requested: show the "Request Pro access" button
function ProUpgradeBlock({ plan, onOpenDialog }: ProUpgradeBlockProps) {
  const { t } = useI18n()
  ...
  if (status.data?.requested) {
    const requestedAt = status.data.requested_at
      ? format(new Date(status.data.requested_at), 'PP')
      : ''
    return (
      <div className="shrink-0 text-right">
        <p className="text-sm font-medium text-zinc-200">{t.pro_settings_already_title}</p>
        <p className="text-xs text-zinc-500 mt-0.5">{t.pro_settings_already_desc.replace('{date}', requestedAt)}</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-lg">
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-900 p-4 flex items-center justify-between gap-4">
          ...
          <ProUpgradeBlock plan={org?.plan} onOpenDialog={() => setShowProDialog(true)} />
        </div>
      </div>
